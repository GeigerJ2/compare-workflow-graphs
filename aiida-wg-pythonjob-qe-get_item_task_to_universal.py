...
# this is the function that is not in the workflow
@task.pythonjob()
...
for i, strain in enumerate(strain_lst):
    ...
    get_item_task = wg.add_task(
        get_item,
        key=str(i),
        data=wg.tasks.generate_structures.outputs.structures,
        register_pickle_by_value=True,
    )
    get_dict_task = wg.add_task(
        get_dict, name=f"get_dict_{i}", register_pickle_by_value=True
    )
    wg.add_link(get_item_task.outputs.result, get_dict_task.inputs.kwargs)
